...
# 强制设置 stdout/stderr 编码为 UTF-8（解决 Windows 终端乱码）
...
def main():
    ...
    try:
        ...
        print(f"\n[OK] Excel 文件: {file_path}")
        print(f"[TXT] 描述文件: {desc_path}")
        print(f"[DATA] 总行数: {total_rows}, 表数: {len(tables)}")

        # Save original JSON
        json_filename = output_dir / f"mx_data_{safe_filename(query)}_raw.json"
        with open(json_filename, "w", encoding="utf-8") as f:
            json.dump(result, f, ensure_ascii=False, indent=2)
        print(f"[JSON] 原始JSON: {json_filename}")
